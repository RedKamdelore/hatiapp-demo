#!/usr/bin/env python3
"""
Импорт волонтёров из Excel-анкет (финальная версия).
username = ТГ (сохраняем регистр), full_name = Позывной, password = Телефон.
Всего должно быть 93 волонтёра.
"""
import sys
...
def get_dates(row_data, date_columns, headers):
    yes_dates = []
...
def process():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        # Создаём нового пользователя (всех удалили)
        user = models.User(
            username=username,
            full_name=full_name,
